...
class SearchResultManager {
    ...
    private var sortedDataFromCache: [String:[BookSearchModel]] = [:]
    ...
    private func getSearchResultFromDisk(keyword: String, page: Int, completion: @escaping (BookSearchModel?) -> ()) {
        if let data = sortedDataFromCache[keyword], data.count >= page {
            completion(data[page-1])
        }else {
            completion(nil)
        }
    }
    ...
    //MARK: called once when app start
    public func loadDataFromCoreData(){
        let context = CoreDataManager.shared.persistentContainer.viewContext
        do {
            /// load from core data once and store in global variable
            let dataFromDiskCache = try context.fetch(SearchResultEntity.fetchRequest()) as! [SearchResultEntity]
            for item in dataFromDiskCache {
                let keyword = item.keyword
                let page = item.page
                let total = item.total
                var books = [BookModel]()
                for book in item.books! { //TODO: decoding book by loop, need to fix
                ...
                if sortedDataFromCache[keyword!] == nil {
                    sortedDataFromCache[keyword!] = [BookSearchModel(total: total, page: page ,books: books)]
                }else {
                    sortedDataFromCache[keyword!]?.append(BookSearchModel(total: total, page: page ,books: books))
                }

            }
            for item in sortedDataFromCache {
                print(item.key)
